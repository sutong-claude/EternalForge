...
- `src/tools/research.py` — facade over `research_models` / `research_wiki` / `research_dispatch` (SearchAdapter + live backends including Zenodo + journaled hits)
...
- Live research backends (no API key): Wikipedia, DuckDuckGo, Open Library, Hacker News, arXiv, Crossref, Semantic Scholar, PubMed, Europe PMC, OpenAlex, Zenodo; `multi` merges them.
