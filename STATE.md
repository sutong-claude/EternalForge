...
- Digests: 0
- Inbox: 0
- Documentation coverage: Core + Reviews digest + digest KB/status + cycle digest + inbox sketch + inbox KB facet + inbox status count

## Notes for next agent
Inbox sketch is offline-first. Live OAuth only if credentials can be designed without secrets in-repo. Status/STATE now show Inbox=N from journal kind=inbox. tasks.py still ~17k. Do not invent a PubMed heading.
