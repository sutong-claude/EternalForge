...
- Digests: 0
- Inbox: 0
- Documentation coverage: Core + inbox live client + refresh grant + env-only client credentials + tasks split + daily and weekly review on cycle

## Notes for next agent
Daily and weekly reviews now write on each non-dry cycle (before digest). Public API: `write_cycle_daily` and `write_cycle_weekly` on `tools.review`. Next optional work: split kb.py/research.py/review.py if they near 20k. Do not invent a PubMed heading. When pushing large files through GitHub tools, split modules rather than one huge blob.
