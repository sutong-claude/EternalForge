"""OpenAlex works-by-best_oa_location.source.host_organization extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.host_organization"


class OaBestHostAdapter:
    """OpenAlex works search grouped by best_oa_location.source.host_organization. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabesthost_payload(payload, limit=limit, query=q)
...
def parse_oabesthost_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.host_organization JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.host_organization"
        hits.append(
            Hit(
                title=f"{label} works",
