"""OpenAlex works-by-has_oa_any_repository extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_any_repository"


class OaHasOaAnyRepoAdapter:
    """OpenAlex works search grouped by has_oa_any_repository. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoanyrepo_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {
        ...
        "no",
        "0",
        "not has oa any repository",
        "no any repository",
        "without any repository",
        "without any repo",
        "missing any repository",
        "no repository copy",
    }:
        return False
    return None
...
def _anyrepo_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoanyrepo_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_any_repository JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_any_repository"
        title = (
            "Works with any OA repository"
            if flag
            else "Works without any OA repository"
        )
        hits.append(
            Hit(
                title=title,
                url=_anyrepo_url(flag, query),
                snippet=snippet,
                source="oahasoanyrepo",
            )
        )
    return hits[:limit]
