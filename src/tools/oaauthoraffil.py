"""OpenAlex works-by-authorships.raw_affiliation_string extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.raw_affiliation_string"


class OaAuthorAffilAdapter:
    """OpenAlex works search grouped by authorships.raw_affiliation_string. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthoraffil_payload(payload, limit=limit, query=q)
...
def parse_oaauthoraffil_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.raw_affiliation_string JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.raw_affiliation_string"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_affil_url(key, query),
                snippet=snippet,
                source="oaauthoraffil",
            )
        )
    return hits[:limit]
