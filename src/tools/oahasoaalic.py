"""OpenAlex works-by-has_oa_license extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_license"


class OaHasOaLicAdapter:
    """OpenAlex works search grouped by has_oa_license. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoaalic_payload(payload, limit=limit, query=q)
...
def parse_oahasoaalic_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_license JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_license"
        title = (
            "Works with OA license"
            if flag
            else "Works without OA license"
        )
        hits.append(
            Hit(
                title=title,
