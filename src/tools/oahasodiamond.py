"""OpenAlex works-by-has_oa_diamond extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_diamond"


class OaHasOaDiamondAdapter:
    """OpenAlex works search grouped by has_oa_diamond. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasodiamond_payload(payload, limit=limit, query=q)
...
def _oadia_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasodiamond_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_diamond JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_diamond"
        title = (
            "Works with diamond OA"
            if flag
            else "Works without diamond OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oadia_url(flag, query),
                snippet=snippet,
                source="oahasodiamond",
            )
        )
    return hits[:limit]
