"""OpenAlex works-by-topics.domain.id extras SearchAdapter (group_by; no key)."""
...
FIELD = "topics.domain.id"


class OaTDomainAdapter:
    """OpenAlex works search grouped by topics.domain.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oatdomain_payload(payload, limit=limit, query=q)
...
def _normalize_domain_id(raw: object) -> str:
    ...
    if text.lower().startswith("domain:"):
...
def _domain_label(row: dict) -> str:
    raw = (
        ...
        or row.get("domain")
        or row.get("domain_display_name")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
...
def _domain_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("domain") or row.get(FIELD) or ""
...
def _oa_url(domain_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(domain_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oatdomain_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by topics.domain.id JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _domain_label(row)
        key = _domain_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by topics.domain.id"
        hits.append(
            Hit(
                title=f"{label} works",
