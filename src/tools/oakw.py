"""OpenAlex works-by-topics.keywords extras SearchAdapter (group_by; no key)."""
...
class OaKwAdapter:
    """OpenAlex works search grouped by topics.keywords. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oakw_payload(payload, limit=limit, query=q)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("topics_keywords")
...
def parse_oakw_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by topics.keywords JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by topics.keywords"
        hits.append(
            Hit(
                title=f"{label} works",
