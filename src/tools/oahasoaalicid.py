"""OpenAlex works-by-has_oa_license_id extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_license_id"


class OaHasOaLicIdAdapter:
    """OpenAlex works search grouped by has_oa_license_id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoaalicid_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {
        ...
        "no",
        "0",
        "not has oa license id",
        "no oa license id",
        "without license id",
        "without oa license id",
        "missing license id",
        "no license id",
        "no oa copy license id",
        "unlicensed id",
    }:
        return False
    return None
...
def parse_oahasoaalicid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_license_id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_license_id"
        title = (
            "Works with OA license id"
            if flag
            else "Works without OA license id"
        )
        hits.append(
            Hit(
                title=title,
