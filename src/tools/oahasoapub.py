"""OpenAlex works-by-has_oa_publisher extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_publisher"


class OaHasOaPubAdapter:
    """OpenAlex works search grouped by has_oa_publisher. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoapub_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {
        ...
        "no",
        "0",
        "not has oa publisher",
        "no oa publisher",
        "without publisher",
        "without oa publisher",
        "missing publisher",
        "no publisher copy",
    }:
        return False
...
def _pub_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoapub_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_publisher JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_publisher"
        title = (
            "Works with OA publisher copy"
            if flag
            else "Works without OA publisher copy"
        )
        hits.append(
            Hit(
                title=title,
                url=_pub_url(flag, query),
                snippet=snippet,
                source="oahasoapub",
            )
        )
