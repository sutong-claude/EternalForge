"""OpenAlex works-by-locations.source.is_in_doaj extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.is_in_doaj"


class OaLocDoajAdapter:
    """OpenAlex works search grouped by locations.source.is_in_doaj. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocdoaj_payload(payload, limit=limit, query=q)
...
def parse_oalocdoaj_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.is_in_doaj JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.is_in_doaj"
        title = (
            "Works with a location source in DOAJ"
            if flag
            else "Works without a location source in DOAJ"
        )
        hits.append(
            Hit(
                title=title,
