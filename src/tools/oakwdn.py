"""OpenAlex works-by-keywords.display_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "keywords.display_name"


class OaKwDnAdapter:
    """OpenAlex works search grouped by keywords.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oakwdn_payload(payload, limit=limit, query=q)
...
def _normalize_keyword_name(raw: object) -> str:
...
def _keyword_label(row: dict) -> str:
    ...
    return _normalize_keyword_name(raw)
...
def parse_oakwdn_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by keywords.display_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by keywords.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
