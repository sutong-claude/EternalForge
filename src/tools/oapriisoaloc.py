"""OpenAlex works-by-primary_location.is_oa extras SearchAdapter (group_by; no key)."""
...
class OaPriIsOaLocAdapter:
    """OpenAlex works search grouped by primary_location.is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oapriisoaloc_payload(payload, limit=limit, query=q)
...
def parse_oapriisoaloc_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_location.is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_location.is_oa"
        title = (
            "Works whose primary location is OA"
            if flag
            else "Works whose primary location is not OA"
        )
        hits.append(
            Hit(
                title=title,
