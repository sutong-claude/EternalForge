"""OpenAlex works-by-authorships.author_position extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.author_position"


class OaAuthorPosAdapter:
    """OpenAlex works search grouped by authorships.author_position. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthorpos_payload(payload, limit=limit, query=q)
...
def _pos_label(row: dict) -> str:
    raw = (
        row.get("key_display_name")
        or row.get("author_position")
        or row.get("position")
        or row.get(FIELD)
        or row.get("key")
        or ""
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("author_positions")
...
def _pos_url(pos_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(pos_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaauthorpos_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.author_position JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _pos_label(row)
        key = _pos_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.author_position"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_pos_url(key, query),
                snippet=snippet,
                source="oaauthorpos",
            )
        )
