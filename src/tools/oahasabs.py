"""OpenAlex works-by-has_abstract extras SearchAdapter (group_by has_abstract; no key)."""
...
class OaHasAbsAdapter:
    """OpenAlex works search grouped by has_abstract. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_abstract&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasabs_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    raw = row.get("key")
    if raw is None:
        raw = row.get("has_abstract")
    if raw is None:
        raw = row.get("abstract")
    if raw is None:
...
def _abs_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_abstract:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oahasabs_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_abstract JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_abstract"
        title = "Works with abstract" if flag else "Works without abstract"
        hits.append(
            Hit(
                title=title,
                url=_abs_url(flag, query),
                snippet=snippet,
                source="oahasabs",
            )
        )
