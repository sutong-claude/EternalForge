"""OpenAlex works-by-best_oa_location.source.issn_l extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.issn_l"


class OaBestIssnAdapter:
    """OpenAlex works search grouped by best_oa_location.source.issn_l. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestissn_payload(payload, limit=limit, query=q)
...
def _issn_key(row: dict) -> str:
    raw = row.get("key") or row.get("issn_l") or row.get("issn") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("issn_ls")
        or payload.get("issns")
...
def _issn_url(issn_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(issn_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oabestissn_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.issn_l JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _issn_label(row)
        key = _issn_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.issn_l"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_issn_url(key, query),
                snippet=snippet,
                source="oabestissn",
            )
        )
