"""OpenAlex works-by-authorships.raw_affiliation_strings extras SearchAdapter (group_by; no key)."""
...
class OaAuthorAffilsAdapter:
    """OpenAlex works search grouped by authorships.raw_affiliation_strings. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthoraffils_payload(payload, limit=limit, query=q)
...
def _affil_label(row: dict) -> str:
    raw = (
        row.get("key_display_name")
        or row.get("raw_affiliation_strings")
        or row.get("raw_affiliation_string")
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("raw_affiliation_strings")
...
def _affil_url(affil_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(affil_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaauthoraffils_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.raw_affiliation_strings JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _affil_label(row)
        key = _affil_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.raw_affiliation_strings"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_affil_url(key, query),
                snippet=snippet,
                source="oaauthoraffils",
            )
        )
    return hits[:limit]
