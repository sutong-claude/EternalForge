"""OpenAlex works-by-best_oa_location.version extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.version"


class OaBestVerAdapter:
    """OpenAlex works search grouped by best_oa_location.version. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestver_payload(payload, limit=limit, query=q)
...
def _ver_label(row: dict) -> str:
    raw = row.get("key_display_name") or row.get("version") or row.get(FIELD) or row.get("key") or ""
    text = str(raw).strip()
...
def _ver_key(row: dict) -> str:
    raw = row.get("key") or row.get(FIELD) or row.get("version") or row.get("key_display_name") or ""
...
def _ver_url(ver_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(ver_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oabestver_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.version JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _ver_label(row)
        key = _ver_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.version"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_ver_url(key, query),
                snippet=snippet,
                source="oabestver",
            )
        )
