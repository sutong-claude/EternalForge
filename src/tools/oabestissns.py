"""OpenAlex works-by-best_oa_location.source.issn extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.issn"


class OaBestIssnsAdapter:
    """OpenAlex works search grouped by best_oa_location.source.issn. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestissns_payload(payload, limit=limit, query=q)
...
def _issn_key(row: dict) -> str:
    raw = row.get("key") or row.get("issn") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
...
def parse_oabestissns_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.issn JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.issn"
        hits.append(
            Hit(
                title=f"{label} works",
