"""OpenAlex works-by-sustainable_development_goals extras SearchAdapter (group_by; no key)."""
...
class OaSdgsAdapter:
    """OpenAlex works search grouped by sustainable_development_goals.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oasdgs_payload(payload, limit=limit, query=q)
...
def _normalize_sdg_id(raw: object) -> str:
    text = str(raw or "").strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
        return text.rstrip("/")
    if text.lower().startswith("sdg:"):
        text = text.split(":", 1)[1].strip()
    return text


def _sdg_label(row: dict) -> str:
    raw = (
...
def _sdg_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("sdg") or row.get(FIELD) or ""
    return _normalize_sdg_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("sustainable_development_goals")
        or payload.get("sdgs")
...
def _oa_url(sdg_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(sdg_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oasdgs_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by sustainable_development_goals.id JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _sdg_label(row)
        key = _sdg_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by sustainable_development_goals"
        hits.append(
            Hit(
                title=f"{label} works",
