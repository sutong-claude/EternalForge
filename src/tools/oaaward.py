"""OpenAlex works-by-grants.award_id extras SearchAdapter (group_by; no key)."""
...
FIELD = "grants.award_id"


class OaAwardAdapter:
    """OpenAlex works search grouped by grants.award_id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaaward_payload(payload, limit=limit, query=q)
...
def _normalize_award_id(raw: object) -> str:
    text = str(raw or "").strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
        text = text.rstrip("/").split("/")[-1]
    if text.lower().startswith("award:"):
        text = text.split(":", 1)[1].strip()
    return text


def _award_label(row: dict) -> str:
    raw = (
...
def _award_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("award_id") or row.get("award") or row.get(FIELD) or ""
    return _normalize_award_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("awards")
        or payload.get("award_ids")
...
def _oa_url(award_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(award_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaaward_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by grants.award_id JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _award_label(row)
        key = _award_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by grants.award_id"
        hits.append(
            Hit(
                title=f"{label} works",
