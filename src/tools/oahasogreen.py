"""OpenAlex works-by-has_oa_green extras SearchAdapter (group_by; no key)."""
...
class OaHasOaGreenAdapter:
    """OpenAlex works search grouped by has_oa_green. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasogreen_payload(payload, limit=limit, query=q)
...
def _oagreen_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasogreen_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_green JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_green"
        title = (
            "Works with green OA"
            if flag
            else "Works without green OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oagreen_url(flag, query),
                snippet=snippet,
                source="oahasogreen",
            )
        )
