...
def get_adapter(name: str | None = None) -> SearchAdapter:
    ...
    if key in LIVE_EXTRA:
        ...
        from tools import oabestver as oabvermod
        from tools import oabestsrc as oabsrcmod
        ...
        from tools import oabestcore as oabcoremod
        ...
        if key in {"oabestver", "bestver-oa", "works-bestver-oa"}:
            return oabvermod.OaBestVerAdapter()
        if key in {"oabestsrc", "bestsrc-oa", "works-bestsrc-oa"}:
            return oabsrcmod.OaBestSrcAdapter()
        if key in {"oabestsrcid", "bestsrcid-oa", "works-bestsrcid-oa"}:
            return oabsrcidmod.OaBestSrcIdAdapter()
        if key in {"oabestsrctype", "bestsrctype-oa", "works-bestsrctype-oa"}:
        ...
        if key in {"oabesthoname", "besthoname-oa", "works-besthoname-oa"}:
            return oabhnamemod.OaBestHoNameAdapter()
        if key in {"oabestholineage", "bestholineage-oa", "works-bestholineage-oa"}:
        ...
        if key in {"oabestcore", "bestcore-oa", "works-bestcore-oa"}:
            return oabcoremod.OaBestCoreAdapter()
        if key in {"oabestisoa", "bestisoa-oa", "works-bestisoa-oa"}:
            return oabisoamod.OaBestIsOaAdapter()
        if key in {"oabestissns", "bestissns-oa", "works-bestissns-oa"}:
