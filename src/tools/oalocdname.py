"""OpenAlex works-by-locations.source.display_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.display_name"


class OaLocDnameAdapter:
    """OpenAlex works search grouped by locations.source.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocdname_payload(payload, limit=limit, query=q)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("source_display_names")
...
def parse_oalocdname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.display_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
