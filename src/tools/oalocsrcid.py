"""OpenAlex works-by-locations.source.id extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.id"


class OaLocSrcIdAdapter:
    """OpenAlex works search grouped by locations.source.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocsrcid_payload(payload, limit=limit, query=q)
...
def parse_oalocsrcid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.id"
        hits.append(
            Hit(
                title=f"{label} works",
