"""OpenAlex works-by-best_oa_location.source.is_core extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.is_core"


class OaBestCoreAdapter:
    """OpenAlex works search grouped by best_oa_location.source.is_core. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestcore_payload(payload, limit=limit, query=q)
...
def parse_oabestcore_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.is_core JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.is_core"
        title = (
            "Works whose best OA source is core"
            if flag
            else "Works whose best OA source is not core"
        )
        hits.append(
            Hit(
                title=title,
