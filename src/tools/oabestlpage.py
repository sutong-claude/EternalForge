"""OpenAlex works-by-best_oa_location.landing_page_url extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.landing_page_url"


class OaBestLpageAdapter:
    """OpenAlex works search grouped by best_oa_location.landing_page_url. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestlpage_payload(payload, limit=limit, query=q)
...
def _page_label(row: dict) -> str:
    raw = row.get("key_display_name") or row.get("landing_page_url") or row.get(FIELD) or row.get("key") or ""
    text = str(raw).strip()
...
def _page_key(row: dict) -> str:
    raw = row.get("key") or row.get(FIELD) or row.get("landing_page_url") or row.get("key_display_name") or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    return text
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("landing_page_urls")
...
def _page_url(page_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(page_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oabestlpage_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.landing_page_url JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _page_label(row)
        key = _page_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.landing_page_url"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_page_url(key, query),
                snippet=snippet,
                source="oabestlpage",
            )
        )
