"""OpenAlex works-by-best_oa_location.source.type extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.type"


class OaBestSrcTypeAdapter:
    """OpenAlex works search grouped by best_oa_location.source.type. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestsrctype_payload(payload, limit=limit, query=q)
...
def parse_oabestsrctype_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.type JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.type"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_src_url(key, query),
                snippet=snippet,
                source="oabestsrctype",
            )
        )
    return hits[:limit]
