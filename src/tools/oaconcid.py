"""OpenAlex works-by-concepts.id extras SearchAdapter (group_by; no key)."""
...
class OaConcIdAdapter:
    """OpenAlex works search grouped by concepts.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaconcid_payload(payload, limit=limit, query=q)
...
def _normalize_concept_id(raw: object) -> str:
...
def _concept_label(row: dict) -> str:
    raw = (
        ...
        or row.get("concept")
        or row.get("concept_display_name")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
...
def _concept_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("concept") or row.get(FIELD) or ""
    return _normalize_concept_id(raw)
...
def _oa_url(concept_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(concept_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaconcid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by concepts.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by concepts.id"
        hits.append(
            Hit(
                title=f"{label} works",
