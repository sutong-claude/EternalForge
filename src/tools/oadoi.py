"""OpenAlex works-by-has_doi extras SearchAdapter (group_by has_doi; no key)."""
...
class OaDoiAdapter:
    """OpenAlex works search grouped by has_doi. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_doi&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oadoi_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {"true", "yes", "1", "has doi", "with doi", "doi"}:
        return True
    if text in {"false", "no", "0", "no doi", "without doi", "missing doi"}:
        return False
    return None
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = payload.get("group_by") or payload.get("has_doi") or payload.get("dois") or []
    if isinstance(rows, list):
        return [item for item in rows if isinstance(item, dict)]
    return []


def _doi_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_doi:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oadoi_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_doi JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_doi"
        title = "Works with DOI" if flag else "Works without DOI"
        hits.append(
            Hit(
                title=title,
                url=_doi_url(flag, query),
                snippet=snippet,
                source="oadoi",
            )
        )
