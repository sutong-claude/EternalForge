"""OpenAlex works-by-best_oa_location.source.is_in_doaj extras SearchAdapter (group_by; no key)."""
...
class OaBestDoajAdapter:
    """OpenAlex works search grouped by best_oa_location.source.is_in_doaj. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestdoaj_payload(payload, limit=limit, query=q)
...
def _doaj_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oabestdoaj_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.is_in_doaj JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.is_in_doaj"
        title = (
            "Works whose best OA source is in DOAJ"
            if flag
            else "Works whose best OA source is not in DOAJ"
        )
        hits.append(
            Hit(
                title=title,
                url=_doaj_url(flag, query),
                snippet=snippet,
                source="oabestdoaj",
            )
        )
