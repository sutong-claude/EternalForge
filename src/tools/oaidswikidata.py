"""OpenAlex works-by-ids.wikidata extras SearchAdapter (group_by; no key)."""
...
class OaIdsWikidataAdapter:
    """OpenAlex works search grouped by ids.wikidata. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaidswikidata_payload(payload, limit=limit, query=q)
...
def _wd_label(row: dict) -> str:
    ...
    return _normalize_wikidata_id(raw)


def _wd_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("wikidata") or row.get(FIELD) or ""
    return _normalize_wikidata_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("ids.wikidata")
        or payload.get("wikidata_ids")
        or payload.get("wikidata")
...
def _oa_url(wd_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(wd_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaidswikidata_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by ids.wikidata JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by ids.wikidata"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_oa_url(key, query),
                snippet=snippet,
                source="oaidswikidata",
            )
        )
    return hits[:limit]
