"""OpenAlex works-by-locations.source.type extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.type"


class OaLocSrcTypeAdapter:
    """OpenAlex works search grouped by locations.source.type. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocsrctype_payload(payload, limit=limit, query=q)
...
def parse_oalocsrctype_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.type JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.type"
        hits.append(
            Hit(
                title=f"{label} works",
