"""OpenAlex works-by-best_oa_location.license_id extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.license_id"


class OaBestLicIdAdapter:
    """OpenAlex works search grouped by best_oa_location.license_id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestlicid_payload(payload, limit=limit, query=q)
...
def parse_oabestlicid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.license_id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.license_id"
        hits.append(
            Hit(
                title=f"{label} works",
