"""OpenAlex works-by-has_fulltext extras SearchAdapter (group_by has_fulltext; no key)."""
...
class OaHasFtAdapter:
    """OpenAlex works search grouped by has_fulltext. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_fulltext&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasft_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    raw = row.get("key")
    if raw is None:
        raw = row.get("has_fulltext")
    if raw is None:
        raw = row.get("fulltext")
    if raw is None:
...
def _ft_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_fulltext:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oahasft_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_fulltext JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_fulltext"
        title = "Works with fulltext" if flag else "Works without fulltext"
        hits.append(
            Hit(
                title=title,
                url=_ft_url(flag, query),
                snippet=snippet,
                source="oahasft",
            )
        )
