"""OpenAlex works-by-concepts.wikidata extras SearchAdapter (group_by; no key)."""
...
FIELD = "concepts.wikidata"


class OaConcWdAdapter:
    """OpenAlex works search grouped by concepts.wikidata. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaconcwd_payload(payload, limit=limit, query=q)
...
def _normalize_wikidata(raw: object) -> str:
...
def _wd_label(row: dict) -> str:
    ...
    return _normalize_wikidata(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("concept_wikidata")
...
def _oa_url(wd_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(wd_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaconcwd_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by concepts.wikidata JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by concepts.wikidata"
        hits.append(
            Hit(
                title=f"{label} works",
