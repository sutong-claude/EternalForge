"""OpenAlex works-by-has_pdf extras SearchAdapter (group_by has_pdf; no key)."""
...
class OaHasPdfAdapter:
    """OpenAlex works search grouped by has_pdf. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_pdf&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahaspdf_payload(payload, limit=limit, query=q)
...
def _pdf_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_pdf:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oahaspdf_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_pdf JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_pdf"
        title = "Works with PDF" if flag else "Works without PDF"
        hits.append(
            Hit(
                title=title,
                url=_pdf_url(flag, query),
                snippet=snippet,
                source="oahaspdf",
            )
        )
