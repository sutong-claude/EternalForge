"""OpenAlex works-by-primary_location.source.is_in_doaj extras SearchAdapter (group_by; no key)."""
...
class OaPriDoajAdapter:
    """OpenAlex works search grouped by primary_location.source.is_in_doaj. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oapridoaj_payload(payload, limit=limit, query=q)
...
def parse_oapridoaj_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_location.source.is_in_doaj JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_location.source.is_in_doaj"
        title = (
            "Works whose primary source is in DOAJ"
            if flag
            else "Works whose primary source is not in DOAJ"
        )
        hits.append(
            Hit(
                title=title,
