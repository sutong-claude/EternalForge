"""OpenAlex works-by-has_oa_hybrid extras SearchAdapter (group_by; no key)."""
...
class OaHasOaHybridAdapter:
    """OpenAlex works search grouped by has_oa_hybrid. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoahybrid_payload(payload, limit=limit, query=q)
...
def _oahybrid_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoahybrid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_hybrid JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_hybrid"
        title = (
            "Works with hybrid OA"
            if flag
            else "Works without hybrid OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oahybrid_url(flag, query),
                snippet=snippet,
                source="oahasoahybrid",
            )
        )
    return hits[:limit]
