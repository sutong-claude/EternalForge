"""OpenAlex works-by-has_oa_accepted_or_published extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_accepted_or_published"


class OaHasOaapAdapter:
    """OpenAlex works search grouped by has_oa_accepted_or_published. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoaap_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if raw is None:
        raw = row.get(FIELD)
    if raw is None:
        raw = row.get("has_accepted_or_published")
    if raw is None:
    ...
    if text in {
        ...
        "yes",
        "1",
        "has oa accepted or published",
        "accepted or published",
        "accepted/published",
        "with accepted or published oa",
        "oa accepted or published",
    }:
        return True
    if text in {
        "false",
        "no",
        "0",
        "no oa accepted or published",
        "without accepted or published oa",
        "missing accepted or published oa",
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("has_accepted_or_published")
        or []
    )
...
def _oaap_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoaap_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_accepted_or_published JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_accepted_or_published"
        title = (
            "Works with accepted or published OA"
            if flag
            else "Works without accepted or published OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oaap_url(flag, query),
                snippet=snippet,
                source="oahasoaap",
            )
        )
