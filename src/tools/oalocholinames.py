"""OpenAlex works-by-locations.source.host_organization_lineage_names extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.host_organization_lineage_names"


class OaLocHoLinamesAdapter:
    """OpenAlex works search grouped by locations.source.host_organization_lineage_names. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocholinames_payload(payload, limit=limit, query=q)
...
def parse_oalocholinames_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.host_organization_lineage_names JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.host_organization_lineage_names"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_name_url(key, query),
                snippet=snippet,
                source="oalocholinames",
            )
        )
    return hits[:limit]
