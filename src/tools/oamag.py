"""OpenAlex works-by-ids.mag extras SearchAdapter (group_by; no key)."""
...
class OaMagAdapter:
    """OpenAlex works search grouped by ids.mag. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oamag_payload(payload, limit=limit, query=q)
...
def _mag_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("mag") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _mag_url(mag_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(mag_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oamag_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by ids.mag JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _mag_label(row)
        key = _mag_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by ids.mag"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_mag_url(key, query),
                snippet=snippet,
                source="oamag",
            )
        )
