"""OpenAlex works-by-authorships.institutions.display_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.institutions.display_name"


class OaAuthDnameAdapter:
    """OpenAlex works search grouped by authorships.institutions.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthdname_payload(payload, limit=limit, query=q)
...
def parse_oaauthdname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.display_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
