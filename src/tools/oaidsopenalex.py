"""OpenAlex works-by-ids.openalex extras SearchAdapter (group_by; no key)."""
...
class OaIdsOpenalexAdapter:
    """OpenAlex works search grouped by ids.openalex. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaidsopenalex_payload(payload, limit=limit, query=q)
...
def _oa_label(row: dict) -> str:
    ...
    return _normalize_openalex_id(raw)


def _oa_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("openalex") or row.get(FIELD) or ""
    return _normalize_openalex_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("ids.openalex")
        or payload.get("openalex_ids")
        or payload.get("openalex")
...
def _oa_url(oa_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(oa_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaidsopenalex_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by ids.openalex JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by ids.openalex"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_oa_url(key, query),
                snippet=snippet,
                source="oaidsopenalex",
            )
        )
    return hits[:limit]
