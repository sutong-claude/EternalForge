"""OpenAlex works-by-has_oa extras SearchAdapter (group_by has_oa; no key)."""
...
class OaHasOaAdapter:
    """OpenAlex works search grouped by has_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_oa&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoa_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {"true", "yes", "1", "has oa", "with oa", "open access", "oa"}:
        return True
    if text in {"false", "no", "0", "no oa", "without oa", "closed", "not oa"}:
        return False
    return None
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = payload.get("group_by") or payload.get("has_oa") or payload.get("oa_flags") or []
    if isinstance(rows, list):
        return [item for item in rows if isinstance(item, dict)]
    return []


def _oa_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_oa:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oahasoa_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa"
        title = "Works with OA" if flag else "Works without OA"
        hits.append(
            Hit(
                title=title,
                url=_oa_url(flag, query),
                snippet=snippet,
                source="oahasoa",
            )
        )
