"""OpenAlex works-by-locations.pdf_url extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.pdf_url"


class OaLocPdfUrlAdapter:
    """OpenAlex works search grouped by locations.pdf_url. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocpdfurl_payload(payload, limit=limit, query=q)
...
def parse_oalocpdfurl_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.pdf_url JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.pdf_url"
        hits.append(
            Hit(
                title=f"{label} works",
