"""OpenAlex works-by-concepts.level extras SearchAdapter (group_by; no key)."""
...
FIELD = "concepts.level"


class OaConcLevAdapter:
    """OpenAlex works search grouped by concepts.level. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaconclev_payload(payload, limit=limit, query=q)
...
def _normalize_level(raw: object) -> str:
    ...
    if text.lower().startswith("concepts.level:"):
        text = text.split(":", 1)[1].strip()
    if text.isdigit() or (text.startswith("-") and text[1:].isdigit()):
        return str(int(text))
    return text
...
def _level_label(row: dict) -> str:
    raw = (
        ...
        or row.get("level")
        or row.get("concept_level")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
    text = _normalize_level(raw)
    if not text:
        return ""
    return f"level {text}"


def _level_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("level") or row.get(FIELD) or ""
...
def _oa_url(level_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(level_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaconclev_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by concepts.level JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _level_label(row)
        key = _level_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by concepts.level"
        hits.append(
            Hit(
                title=f"{label} works",
