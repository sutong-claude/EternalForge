"""OpenAlex works-by-ids.pmcid extras SearchAdapter (group_by; no key)."""
...
class OaPmcidAdapter:
    """OpenAlex works search grouped by ids.pmcid. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oapmcid_payload(payload, limit=limit, query=q)
...
def _pmcid_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("pmcid") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("ids.pmcid")
        or payload.get("pmcids")
...
def _pmcid_url(pmcid_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(pmcid_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oapmcid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by ids.pmcid JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _pmcid_label(row)
        key = _pmcid_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by ids.pmcid"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_pmcid_url(key, query),
                snippet=snippet,
                source="oapmcid",
            )
        )
