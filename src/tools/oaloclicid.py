"""OpenAlex works-by-locations.license_id extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.license_id"


class OaLocLicIdAdapter:
    """OpenAlex works search grouped by locations.license_id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaloclicid_payload(payload, limit=limit, query=q)
...
def parse_oaloclicid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.license_id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.license_id"
        hits.append(
            Hit(
                title=f"{label} works",
