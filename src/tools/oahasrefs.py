"""OpenAlex works-by-has_references extras SearchAdapter (group_by has_references; no key)."""
...
class OaHasRefsAdapter:
    """OpenAlex works search grouped by has_references. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        url = f"{self.endpoint}?search={quote(q)}&group_by=has_references&per-page={limit}"
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasrefs_payload(payload, limit=limit, query=q)
...
def _refs_url(flag: bool, query: str = "") -> str:
    q = query.strip()
    filt = f"has_references:{str(flag).lower()}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oahasrefs_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_references JSON into research Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_references"
        title = "Works with references" if flag else "Works without references"
        hits.append(
            Hit(
                title=title,
                url=_refs_url(flag, query),
                snippet=snippet,
                source="oahasrefs",
            )
        )
