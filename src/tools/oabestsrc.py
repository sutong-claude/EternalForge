"""OpenAlex works-by-best_oa_location.source extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source"


class OaBestSrcAdapter:
    """OpenAlex works search grouped by best_oa_location.source. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestsrc_payload(payload, limit=limit, query=q)
...
def _src_label(row: dict) -> str:
    raw = row.get("key_display_name") or row.get("display_name") or row.get("source") or row.get(FIELD) or row.get("key") or ""
    text = str(raw).strip()
...
def _src_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get(FIELD) or row.get("source") or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def parse_oabestsrc_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source"
        hits.append(
            Hit(
                title=f"{label} works",
