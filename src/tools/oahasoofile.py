"""OpenAlex works-by-has_oa_file extras SearchAdapter (group_by; no key)."""
...
class OaHasOaFileAdapter:
    """OpenAlex works search grouped by has_oa_file. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoofile_payload(payload, limit=limit, query=q)
...
def _file_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoofile_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_file JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_file"
        title = (
            "Works with OA file"
            if flag
            else "Works without OA file"
        )
        hits.append(
            Hit(
                title=title,
                url=_file_url(flag, query),
                snippet=snippet,
                source="oahasoofile",
            )
        )
