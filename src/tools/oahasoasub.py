"""OpenAlex works-by-has_oa_submitted extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_submitted"


class OaHasOaSubAdapter:
    """OpenAlex works search grouped by has_oa_submitted. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoasub_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {
        ...
        "0",
        "no oa submitted",
        "without submitted oa",
        "missing submitted oa",
        "not submitted",
    }:
        return False
    return None
...
def _oasub_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoasub_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_submitted JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_submitted"
        title = (
            "Works with submitted OA"
            if flag
            else "Works without submitted OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oasub_url(flag, query),
                snippet=snippet,
                source="oahasoasub",
            )
        )
