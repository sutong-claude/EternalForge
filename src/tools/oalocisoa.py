"""OpenAlex works-by-locations.is_oa extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.is_oa"


class OaLocIsOaAdapter:
    """OpenAlex works search grouped by locations.is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocisoa_payload(payload, limit=limit, query=q)
...
def parse_oalocisoa_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.is_oa"
        title = (
            "Works with an OA location"
            if flag
            else "Works without an OA location"
        )
        hits.append(
            Hit(
                title=title,
