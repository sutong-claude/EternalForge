"""OpenAlex works-by-best_oa_location.license extras SearchAdapter (group_by; no key)."""
...
class OaBestLicAdapter:
    """OpenAlex works search grouped by best_oa_location.license. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestlic_payload(payload, limit=limit, query=q)
...
def parse_oabestlic_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.license JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.license"
        hits.append(
            Hit(
                title=f"{label} works",
