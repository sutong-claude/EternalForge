"""OpenAlex works-by-authorships.institutions.id extras SearchAdapter (group_by; no key)."""
...
class OaAuthIdAdapter:
    """OpenAlex works search grouped by authorships.institutions.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthid_payload(payload, limit=limit, query=q)
...
def parse_oaauthid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.id"
        hits.append(
            Hit(
                title=f"{label} works",
