"""OpenAlex works-by-best_oa_location.source.display_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.display_name"


class OaBestDnameAdapter:
    """OpenAlex works search grouped by best_oa_location.source.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestdname_payload(payload, limit=limit, query=q)
...
def _dname_key(row: dict) -> str:
    raw = row.get("key") or row.get("display_name") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
...
def _dname_url(dname_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(dname_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oabestdname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.display_name JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _dname_label(row)
        key = _dname_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_dname_url(key, query),
                snippet=snippet,
                source="oabestdname",
            )
        )
