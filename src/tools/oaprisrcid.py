"""OpenAlex works-by-primary_location.source.id extras SearchAdapter (group_by; no key)."""
...
class OaPriSrcIdAdapter:
    """OpenAlex works search grouped by primary_location.source.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaprisrcid_payload(payload, limit=limit, query=q)
...
def parse_oaprisrcid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_location.source.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_location.source.id"
        hits.append(
            Hit(
                title=f"{label} works",
