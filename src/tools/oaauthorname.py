"""OpenAlex works-by-authorships.raw_author_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.raw_author_name"


class OaAuthorNameAdapter:
    """OpenAlex works search grouped by authorships.raw_author_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthorname_payload(payload, limit=limit, query=q)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("raw_author_names")
        or payload.get("author_names")
...
def parse_oaauthorname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.raw_author_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.raw_author_name"
        hits.append(
            Hit(
                title=f"{label} works",
