"""OpenAlex works-by-authorships.institutions.lineage extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.institutions.lineage"


class OaAuthLinAdapter:
    """OpenAlex works search grouped by authorships.institutions.lineage. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthlin_payload(payload, limit=limit, query=q)
...
def parse_oaauthlin_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.lineage JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.lineage"
        hits.append(
            Hit(
                title=f"{label} works",
