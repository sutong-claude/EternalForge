"""OpenAlex works-by-authorships.author.id extras SearchAdapter (group_by; no key)."""
...
class OaAuthorIdAdapter:
    """OpenAlex works search grouped by authorships.author.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthorid_payload(payload, limit=limit, query=q)
...
def parse_oaauthorid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.author.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.author.id"
        hits.append(
            Hit(
                title=f"{label} works",
