"""OpenAlex works-by-topics.id extras SearchAdapter (group_by; no key)."""
...
class OaTopicIdAdapter:
    """OpenAlex works search grouped by topics.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oatopicid_payload(payload, limit=limit, query=q)
...
def _topic_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("topic") or row.get(FIELD) or ""
...
def parse_oatopicid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by topics.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by topics.id"
        hits.append(
            Hit(
                title=f"{label} works",
