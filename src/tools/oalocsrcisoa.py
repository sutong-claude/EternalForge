"""OpenAlex works-by-locations.source.is_oa extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.is_oa"


class OaLocSrcIsOaAdapter:
    """OpenAlex works search grouped by locations.source.is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocsrcisoa_payload(payload, limit=limit, query=q)
...
def parse_oalocsrcisoa_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.is_oa"
        title = (
            "Works with an OA location source"
            if flag
            else "Works without an OA location source"
        )
        hits.append(
            Hit(
                title=title,
