"""OpenAlex works-by-concepts.display_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "concepts.display_name"


class OaConcDnAdapter:
    """OpenAlex works search grouped by concepts.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaconcdn_payload(payload, limit=limit, query=q)
...
def _normalize_concept_name(raw: object) -> str:
    text = str(raw or "").strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
        text = text.rstrip("/").split("/")[-1]
    if text.lower().startswith("concept:"):
        text = text.split(":", 1)[1].strip()
    return text.replace("+", " ").replace("%20", " ").replace("-", " ").strip()


def _concept_label(row: dict) -> str:
    raw = (
    ...
    return _normalize_concept_name(raw)


def _concept_key(row: dict) -> str:
    raw = (
        row.get("key")
        or row.get("display_name")
...
def _oa_url(name: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(name, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaconcdn_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by concepts.display_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by concepts.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
