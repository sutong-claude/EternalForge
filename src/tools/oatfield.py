"""OpenAlex works-by-topics.field.id extras SearchAdapter (group_by; no key)."""
...
class OaTFieldAdapter:
    """OpenAlex works search grouped by topics.field.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oatfield_payload(payload, limit=limit, query=q)
...
def _normalize_field_id(raw: object) -> str:
...
def _field_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("field") or row.get(FIELD) or ""
    return _normalize_field_id(raw)
...
def _oa_url(field_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(field_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oatfield_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by topics.field.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by topics.field.id"
        hits.append(
            Hit(
                title=f"{label} works",
