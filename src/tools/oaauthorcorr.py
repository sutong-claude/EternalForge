"""OpenAlex works-by-authorships.is_corresponding extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.is_corresponding"


class OaAuthorCorrAdapter:
    """OpenAlex works search grouped by authorships.is_corresponding. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthorcorr_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if raw is None:
        raw = row.get(FIELD)
    if raw is None:
        raw = row.get("is_corresponding")
    if raw is None:
        raw = row.get("corresponding")
    if raw is None:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("is_corresponding")
        or payload.get("corresponding")
...
def _corr_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oaauthorcorr_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.is_corresponding JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.is_corresponding"
        title = (
            "Works with a corresponding authorship"
            if flag
            else "Works without a corresponding authorship"
        )
        hits.append(
            Hit(
                title=title,
                url=_corr_url(flag, query),
                snippet=snippet,
                source="oaauthorcorr",
            )
        )
