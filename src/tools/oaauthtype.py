"""OpenAlex works-by-authorships.institutions.type extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.institutions.type"


class OaAuthTypeAdapter:
    """OpenAlex works search grouped by authorships.institutions.type. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthtype_payload(payload, limit=limit, query=q)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("types")
        or payload.get("institution_types")
...
def _type_url(type_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(type_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaauthtype_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.type JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _type_label(row)
        key = _type_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.type"
        hits.append(
            Hit(
                title=f"{label} works",
