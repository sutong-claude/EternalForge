"""OpenAlex works-by-locations.source.issn extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.issn"


class OaLocIssnsAdapter:
    """OpenAlex works search grouped by locations.source.issn. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocissns_payload(payload, limit=limit, query=q)
...
def parse_oalocissns_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.issn JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.issn"
        hits.append(
            Hit(
                title=f"{label} works",
