"""OpenAlex works-by-has_oa_domain extras SearchAdapter (group_by; no key)."""
...
class OaHasOaDomainAdapter:
    """OpenAlex works search grouped by has_oa_domain. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoadomain_payload(payload, limit=limit, query=q)
...
def _domain_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoadomain_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_domain JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_domain"
        title = (
            "Works with OA domain"
            if flag
            else "Works without OA domain"
        )
        hits.append(
            Hit(
                title=title,
                url=_domain_url(flag, query),
                snippet=snippet,
                source="oahasoadomain",
            )
        )
    return hits[:limit]
