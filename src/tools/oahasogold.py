"""OpenAlex works-by-has_oa_gold extras SearchAdapter (group_by; no key)."""
...
class OaHasOaGoldAdapter:
    """OpenAlex works search grouped by has_oa_gold. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasogold_payload(payload, limit=limit, query=q)
...
def _oagold_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasogold_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_gold JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_gold"
        title = (
            "Works with gold OA"
            if flag
            else "Works without gold OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oagold_url(flag, query),
                snippet=snippet,
                source="oahasogold",
            )
        )
