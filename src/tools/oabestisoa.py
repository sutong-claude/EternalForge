"""OpenAlex works-by-best_oa_location.source.is_oa extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.is_oa"


class OaBestIsOaAdapter:
    """OpenAlex works search grouped by best_oa_location.source.is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestisoa_payload(payload, limit=limit, query=q)
...
def _isoa_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oabestisoa_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.is_oa"
        title = (
            "Works whose best OA source is OA"
            if flag
            else "Works whose best OA source is not OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_isoa_url(flag, query),
                snippet=snippet,
                source="oabestisoa",
            )
        )
