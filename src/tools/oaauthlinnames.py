"""OpenAlex works-by-authorships.institutions.lineage_names extras SearchAdapter (group_by; no key)."""
...
FIELD = "authorships.institutions.lineage_names"


class OaAuthLinNamesAdapter:
    """OpenAlex works search grouped by authorships.institutions.lineage_names. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthlinnames_payload(payload, limit=limit, query=q)
...
def _name_label(row: dict) -> str:
    raw = (
        ...
        or row.get("lineage_name")
        or row.get("lineage_names")
...
def parse_oaauthlinnames_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.lineage_names JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.lineage_names"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_name_url(key, query),
                snippet=snippet,
                source="oaauthlinnames",
            )
        )
    return hits[:limit]
