"""OpenAlex works-by-has_oa_version extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_version"


class OaHasOaVerAdapter:
    """OpenAlex works search grouped by has_oa_version. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoaver_payload(payload, limit=limit, query=q)
...
def parse_oahasoaver_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_version JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_version"
        title = (
            "Works with OA version"
            if flag
            else "Works without OA version"
        )
        hits.append(
            Hit(
                title=title,
