"""OpenAlex works-by-corresponding_institution_ids extras SearchAdapter (group_by; no key)."""
...
class OaCorrInstAdapter:
    """OpenAlex works search grouped by corresponding_institution_ids. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oacorrinst_payload(payload, limit=limit, query=q)
...
def _inst_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get(FIELD) or row.get("institution") or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("corresponding_institution_ids")
        or payload.get("institution_ids")
...
def _inst_url(inst_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(inst_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oacorrinst_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by corresponding_institution_ids JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _inst_label(row)
        key = _inst_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by corresponding_institution_ids"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_inst_url(key, query),
                snippet=snippet,
                source="oacorrinst",
            )
        )
