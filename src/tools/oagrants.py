"""OpenAlex works-by-grants.funder extras SearchAdapter (group_by; no key)."""
...
class OaGrantsAdapter:
    """OpenAlex works search grouped by grants.funder. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oagrants_payload(payload, limit=limit, query=q)
...
def _normalize_funder_id(raw: object) -> str:
    text = str(raw or "").strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
        return text.rstrip("/").split("/")[-1]
    if text.lower().startswith("funder:"):
        text = text.split(":", 1)[1].strip()
    return text


def _funder_label(row: dict) -> str:
    raw = (
...
def _funder_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("funder") or row.get(FIELD) or ""
    return _normalize_funder_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("grants")
        or payload.get("funders")
...
def _oa_url(funder_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(funder_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oagrants_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by grants.funder JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _funder_label(row)
        key = _funder_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by grants.funder"
        hits.append(
            Hit(
                title=f"{label} works",
