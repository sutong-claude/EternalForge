"""OpenAlex works-by-has_oa_repository extras SearchAdapter (group_by; no key)."""
...
FIELD = "has_oa_repository"


class OaHasOaRepoAdapter:
    """OpenAlex works search grouped by has_oa_repository. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoarepo_payload(payload, limit=limit, query=q)
...
def _flag(row: dict) -> bool | None:
    ...
    if text in {
        ...
        "0",
        "no oa repository",
        "without repository oa",
        "missing repository oa",
        "not repository",
    }:
        return False
    return None
...
def _oarepo_url(flag: bool, query: str = "") -> str:
    q = query.strip()
...
def parse_oahasoarepo_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_repository JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_repository"
        title = (
            "Works with repository OA"
            if flag
            else "Works without repository OA"
        )
        hits.append(
            Hit(
                title=title,
                url=_oarepo_url(flag, query),
                snippet=snippet,
                source="oahasoarepo",
            )
        )
