"""OpenAlex works-by-is_oa extras SearchAdapter (group_by; no key)."""
...
class OaIsOaAdapter:
    """OpenAlex works search grouped by is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaisoa_payload(payload, limit=limit, query=q)
...
def parse_oaisoa_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by is_oa"
        title = (
            "Works that are OA"
            if flag
            else "Works that are not OA"
        )
        hits.append(
            Hit(
                title=title,
