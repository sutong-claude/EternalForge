"""OpenAlex works-by-locations.source.host_organization extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.source.host_organization"


class OaLocHostAdapter:
    """OpenAlex works search grouped by locations.source.host_organization. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalochost_payload(payload, limit=limit, query=q)
...
def parse_oalochost_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.source.host_organization JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.source.host_organization"
        hits.append(
            Hit(
                title=f"{label} works",
