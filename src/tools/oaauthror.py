"""OpenAlex works-by-authorships.institutions.ror extras SearchAdapter (group_by; no key)."""
...
class OaAuthRorAdapter:
    """OpenAlex works search grouped by authorships.institutions.ror. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthror_payload(payload, limit=limit, query=q)
...
def _ror_key(row: dict) -> str:
    raw = row.get("key") or row.get("ror") or row.get("id") or row.get(FIELD) or row.get("institution") or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _ror_url(ror_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(ror_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaauthror_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.institutions.ror JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _ror_label(row)
        key = _ror_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.institutions.ror"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_ror_url(key, query),
                snippet=snippet,
                source="oaauthror",
            )
        )
