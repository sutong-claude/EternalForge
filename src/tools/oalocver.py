"""OpenAlex works-by-locations.version extras SearchAdapter (group_by; no key)."""
...
class OaLocVerAdapter:
    """OpenAlex works search grouped by locations.version. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oalocver_payload(payload, limit=limit, query=q)
...
def parse_oalocver_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.version JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.version"
        hits.append(
            Hit(
                title=f"{label} works",
