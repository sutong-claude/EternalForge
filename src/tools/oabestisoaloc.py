"""OpenAlex works-by-best_oa_location.is_oa extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.is_oa"


class OaBestIsOaLocAdapter:
    """OpenAlex works search grouped by best_oa_location.is_oa. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabestisoaloc_payload(payload, limit=limit, query=q)
...
def parse_oabestisoaloc_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.is_oa JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.is_oa"
        title = (
            "Works whose best OA location is OA"
            if flag
            else "Works whose best OA location is not OA"
        )
        hits.append(
            Hit(
                title=title,
