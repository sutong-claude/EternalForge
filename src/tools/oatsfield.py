"""OpenAlex works-by-topics.subfield.id extras SearchAdapter (group_by; no key)."""
...
FIELD = "topics.subfield.id"


class OaTsFieldAdapter:
    """OpenAlex works search grouped by topics.subfield.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oatsfield_payload(payload, limit=limit, query=q)
...
def _normalize_subfield_id(raw: object) -> str:
    text = str(raw or "").strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if text.startswith("http") and "/" in text:
        text = text.rstrip("/").split("/")[-1]
    if text.lower().startswith("subfield:"):
        text = text.split(":", 1)[1].strip()
    return text


def _subfield_label(row: dict) -> str:
    raw = (
        row.get("key_display_name")
        or row.get("display_name")
        or row.get("subfield")
        or row.get("subfield_display_name")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
...
def _subfield_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("subfield") or row.get(FIELD) or ""
    return _normalize_subfield_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("subfields")
        or payload.get("subfield_ids")
...
def _oa_url(subfield_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(subfield_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oatsfield_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by topics.subfield.id JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _subfield_label(row)
        key = _subfield_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by topics.subfield.id"
        hits.append(
            Hit(
                title=f"{label} works",
