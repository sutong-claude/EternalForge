"""OpenAlex works-by-primary_topic.id extras SearchAdapter (group_by; no key)."""
...
class OaPrTopicAdapter:
    """OpenAlex works search grouped by primary_topic.id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaprtopic_payload(payload, limit=limit, query=q)
...
def _normalize_topic_id(raw: object) -> str:
...
def _topic_label(row: dict) -> str:
    raw = (
        ...
        or row.get("primary_topic")
        or row.get("topic")
        or row.get("topic_display_name")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
...
def _topic_key(row: dict) -> str:
    ...
    return _normalize_topic_id(raw)
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("primary_topics")
        or payload.get("topics")
        or payload.get("topic_ids")
...
def _oa_url(topic_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(topic_key, safe='')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaprtopic_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_topic.id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_topic.id"
        hits.append(
            Hit(
                title=f"{label} works",
