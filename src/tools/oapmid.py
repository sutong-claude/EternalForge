"""OpenAlex works-by-ids.pmid extras SearchAdapter (group_by; no key)."""
...
class OaPmidAdapter:
    """OpenAlex works search grouped by ids.pmid. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oapmid_payload(payload, limit=limit, query=q)
...
def _pmid_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get("pmid") or row.get(FIELD) or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("ids.pmid")
        or payload.get("pmids")
        or payload.get("pmid")
...
def _pmid_url(pmid_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(pmid_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oapmid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by ids.pmid JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _pmid_label(row)
        key = _pmid_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by ids.pmid"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_pmid_url(key, query),
                snippet=snippet,
                source="oapmid",
            )
        )
