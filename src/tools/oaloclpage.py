"""OpenAlex works-by-locations.landing_page_url extras SearchAdapter (group_by; no key)."""
...
FIELD = "locations.landing_page_url"


class OaLocLpageAdapter:
    """OpenAlex works search grouped by locations.landing_page_url. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaloclpage_payload(payload, limit=limit, query=q)
...
def parse_oaloclpage_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by locations.landing_page_url JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by locations.landing_page_url"
        hits.append(
            Hit(
                title=f"{label} works",
