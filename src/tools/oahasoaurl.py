"""OpenAlex works-by-has_oa_url extras SearchAdapter (group_by; no key)."""
...
class OaHasOaUrlAdapter:
    """OpenAlex works search grouped by has_oa_url. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oahasoaurl_payload(payload, limit=limit, query=q)
...
def parse_oahasoaurl_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by has_oa_url JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by has_oa_url"
        title = (
            "Works with OA URL"
            if flag
            else "Works without OA URL"
        )
        hits.append(
            Hit(
                title=title,
