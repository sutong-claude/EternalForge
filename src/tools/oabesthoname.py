"""OpenAlex works-by-best_oa_location.source.host_organization_name extras SearchAdapter (group_by; no key)."""
...
FIELD = "best_oa_location.source.host_organization_name"


class OaBestHoNameAdapter:
    """OpenAlex works search grouped by best_oa_location.source.host_organization_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oabesthoname_payload(payload, limit=limit, query=q)
...
def _name_label(row: dict) -> str:
    raw = (
        ...
        or row.get("host_organization_name")
        or row.get(FIELD)
        or row.get("key")
        or ""
    )
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("host_organization_names")
        or payload.get("host_names")
...
def parse_oabesthoname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by best_oa_location.source.host_organization_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by best_oa_location.source.host_organization_name"
        hits.append(
            Hit(
                title=f"{label} works",
