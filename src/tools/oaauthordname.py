"""OpenAlex works-by-authorships.author.display_name extras SearchAdapter (group_by; no key)."""
...
class OaAuthorDnameAdapter:
    """OpenAlex works search grouped by authorships.author.display_name. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthordname_payload(payload, limit=limit, query=q)
...
def parse_oaauthordname_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.author.display_name JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.author.display_name"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_name_url(key, query),
                snippet=snippet,
                source="oaauthordname",
            )
        )
    return hits[:limit]
