"""OpenAlex works-by-primary_location.source.issn extras SearchAdapter (group_by; no key)."""
...
FIELD = "primary_location.source.issn"


class OaPriIssnsAdapter:
    """OpenAlex works search grouped by primary_location.source.issn. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oapriissns_payload(payload, limit=limit, query=q)
...
def parse_oapriissns_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_location.source.issn JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_location.source.issn"
        hits.append(
            Hit(
                title=f"{label} works",
