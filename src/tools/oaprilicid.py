"""OpenAlex works-by-primary_location.license_id extras SearchAdapter (group_by; no key)."""
...
FIELD = "primary_location.license_id"


class OaPriLicIdAdapter:
    """OpenAlex works search grouped by primary_location.license_id. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaprilicid_payload(payload, limit=limit, query=q)
...
def _lic_label(row: dict) -> str:
    raw = row.get("key_display_name") or row.get("license_id") or row.get(FIELD) or row.get("key") or ""
    text = str(raw).strip()
...
def _lic_key(row: dict) -> str:
    raw = row.get("key") or row.get(FIELD) or row.get("license_id") or row.get("key_display_name") or ""
...
def _lic_url(lic_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(lic_key, safe=':/')}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaprilicid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by primary_location.license_id JSON into Hits."""
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by primary_location.license_id"
        hits.append(
            Hit(
                title=f"{label} works",
