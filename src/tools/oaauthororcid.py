"""OpenAlex works-by-authorships.author.orcid extras SearchAdapter (group_by; no key)."""
...
class OaAuthorOrcidAdapter:
    """OpenAlex works search grouped by authorships.author.orcid. No key."""
    ...
    def search(self, query: str, max_results: int = 5) -> list[Hit]:
        ...
        if not isinstance(payload, dict):
            return []
        return parse_oaauthororcid_payload(payload, limit=limit, query=q)
...
def _orcid_key(row: dict) -> str:
    raw = row.get("key") or row.get("id") or row.get(FIELD) or row.get("orcid") or ""
    text = str(raw).strip()
    if text.lower() in {"", "unknown", "null", "none"}:
        return ""
    if "/" in text:
...
def _rows_from_payload(payload: dict) -> list[dict]:
    rows = (
        ...
        or payload.get("orcids")
        or payload.get("authors")
...
def _orcid_url(orcid_key: str, query: str = "") -> str:
    q = query.strip()
    filt = f"{FIELD}:{quote(orcid_key)}"
    if q:
        return f"https://openalex.org/works?search={quote(q)}&filter={filt}"
    return f"https://openalex.org/works?filter={filt}"


def parse_oaauthororcid_payload(payload: dict, limit: int = 5, query: str = "") -> list[Hit]:
    """Map OpenAlex works group_by authorships.author.orcid JSON into Hits."""
    rows: list[tuple[int, dict]] = []
    for row in _rows_from_payload(payload):
        label = _orcid_label(row)
        key = _orcid_key(row)
    ...
    for works, row in rows:
        ...
        snippet = " · ".join(bits) or "OpenAlex works by authorships.author.orcid"
        hits.append(
            Hit(
                title=f"{label} works",
                url=_orcid_url(key, query),
                snippet=snippet,
                source="oaauthororcid",
            )
        )
    return hits[:limit]
